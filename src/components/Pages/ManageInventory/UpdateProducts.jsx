...
function UpdateProducts() {
    ...
    const handleRestockSubmit = async (e) => {
        e.preventDefault();
        const newRestockQuantity =
            parseInt(e.target.quantity.value) + parseInt(product.quantity);
        try {
            setLoading(true);
            await axios.put(
    ...
    return (
        <div>
            ...
            <div className="container mx-auto max-w-[720px] w-[95%] mt-[25px]">
                <img
                    src={product?.url}
                    className="w-[100%] rounded-md mb-3"
                    alt={product?.title}
                />
                ...
                <h5 className="text-2xl font-semibold mb-3 text-pink-700">
                    Price : ${product?.price}
                </h5>
                <h5 className="text-xl mb-3">Quantity : {product?.quantity}</h5>
                {showForm && (
                    <form
                        className="my-3 container mx-auto flex"
                        key={product?._id}
                        onSubmit={handleRestockSubmit}
                    >
                        <input
                            className="py-2 px-4 block w-8/12 rounded-md shadow-md"
                            required
                            type="number"
                        ...
                        <input
                            type="submit"
                            value="Update"
                            className="btn rounded-md text-white shadow-md w-4/12 bg-blue-600 hover:bg-blue-900"
                        />
                    </form>
                )}
                <div className="flex gap-[24px] mb-[140px]">
                    <button
                        className="btn bg-pink-700 rounded-md hover:bg-pink-900 text-white"
                        onClick={() => handleDeliverProduct(product?._id)}
                    >
                        Deliver
          </button>
                    <button
                        className="btn bg-blue-600 hover:bg-blue-900 text-white rounded-md"
