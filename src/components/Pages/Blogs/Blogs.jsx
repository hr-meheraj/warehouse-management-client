...
function Blogs() {
    const [blogs, loading, err] = useFetch('https://mern-stack-inventory-management.hrmeheraj.repl.co/blogs');
    return (
                ...
                blogs?.map(blog => {
                    ...
                    return (
                        <Link to={`${_id}`} className='flex w-full mb-4' key={_id}>
                            ...
                            <img class="h-[250px] w-full  rounded-t-lg md:h-[100%] md:w-48 md:rounded-none md:rounded-l-lg" src={imgUrl} alt={question} />
                            <div class="flex flex-col justify-between p-4 leading-normal">
                                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{question}</h5>
                                <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">{answer?.slice(0, 50)}</p>
                            </div>
                            </div>
                        </Link>
